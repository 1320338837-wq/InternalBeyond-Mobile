# Internal Beyond · Mobile（IB-Mobile）
...
| **DIY** | 外部工具（HTTP 接口）与 MCP 服务器接入，支持调用前确认 |
...
Visual 分基础 / 文字 / 气泡 / 桌面四区，含全站色调整套更换与 4 个视觉方案槽；手机端视觉偏好独立于电脑端。DIY 配置外部 HTTP 工具与 MCP 服务器（浏览器直发请求，目标接口需允许 CORS）。API 页最多管理多个端口，各有昵称、关系、提示词与逐项权限；Presence 定时推送支持固定间隔 / 时段随机与勿扰时段。
